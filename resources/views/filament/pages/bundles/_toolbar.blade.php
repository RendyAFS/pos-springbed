<div class="flex flex-wrap items-center gap-3 mb-6">
    ...
    <div class="relative flex-1 max-w-xs">
        <span class="absolute inset-y-0 left-3 flex items-center pointer-events-none">
            <x-heroicon-o-magnifying-glass class="w-4 h-4 text-gray-400 dark:text-gray-500" wire:loading.class="opacity-0"
                wire:target="bundleSearch" />
            <svg wire:loading wire:target="bundleSearch" class="animate-spin w-4 h-4 text-primary-500 absolute"
        ...
        <input type="text" wire:model.live.debounce.400ms="bundleSearch" placeholder="Search bundles…"
            class="w-full pl-9 pr-10 py-2 rounded-lg
        ...
        @if ($bundleSearch)
            <button wire:click="$set('bundleSearch', '')"
    ...
    {{-- Filter pills --}}
    <div
        class="flex items-center gap-1 p-1 rounded-lg border border-gray-200 dark:border-white/10 bg-gray-50 dark:bg-white/5">
        @foreach ([
        'active' => ['label' => 'Active', 'icon' => 'heroicon-o-check-circle'],
        'trashed' => ['label' => 'Deleted', 'icon' => 'heroicon-o-trash'],
        'all' => ['label' => 'All', 'icon' => 'heroicon-o-squares-2x2'],
    ] as $value => $cfg)
            <button wire:click="$set('bundleFilter', '{{ $value }}')" type="button"
                @class([

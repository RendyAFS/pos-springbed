...
<div @class([
    'relative rounded-xl border p-5 shadow flex flex-col gap-4 transition-transform duration-200 hover:shadow-lg hover:-translate-y-1',
    $cfg['bg'],
    $cfg['border'],
    'opacity-50 ring-1 ring-inset ring-danger-300 dark:ring-danger-800' => $promo->trashed(),
])>

    {{-- ── Trashed badge ──────────────────────────────────────────────── --}}
    @if ($promo->trashed())
        <span
            class="absolute top-3 right-3 inline-flex items-center gap-1 px-2 py-0.5 rounded-full
                     text-xs font-semibold bg-danger-100 text-danger-700 dark:bg-danger-900 dark:text-danger-300">
    ...
    <div class="flex justify-between items-start">
        ...
        <div x-data="{ open: false }" class="relative shrink-0">
            ...
            <div x-show="open" x-transition:enter="transition ease-out duration-100"
                ...
                @if ($promo->trashed())
                    {{-- Restore → Filament Action modal --}}
                    <button type="button" wire:click="mountAction('restorePromo', { id: {{ $promo->id }} })"
                        @click="open = false"
                ...
                @else
                    {{-- Delete → Filament Action modal (style konsisten!) --}}
                    <button type="button" wire:click="mountAction('deletePromo', { id: {{ $promo->id }} })"
                        @click="open = false"
                        class="flex items-center gap-2 px-4 py-2 text-sm w-full text-left
    ...
    <div class="flex flex-col gap-1 text-sm">
        <div class="flex justify-between items-center">
            <span class="text-gray-500 dark:text-gray-400">Min. Pembelian</span>
    ...
    <div class="flex justify-between items-center">
        ...
        <button type="button" wire:click="toggleActive({{ $promo->id }})" wire:loading.attr="disabled"
            wire:target="toggleActive({{ $promo->id }})" @disabled($promo->trashed())
            aria-label="{{ $promo->is_active ? 'Nonaktifkan' : 'Aktifkan' }} promosi" @class([
